...
function PlanCard({ plan, onChoose }) {
  ...
  const isStandard = plan.slug === "standard" || plan.name?.toLowerCase() === "standard";

  return (
    <article className={getPlanCardClass(variant)}>
      {badge && (
        <span className={`absolute left-1/2 top-5 inline-flex min-h-[30px] min-w-[150px] -translate-x-1/2 items-center justify-center rounded-full px-[18px] text-xs font-black ${isPremium ? "bg-[#ffd54f] text-[#111]" : "bg-[#e6002e] text-white"}`}>
          {badge}
        </span>
      )}
      <h2 className="mb-2.5 mt-0 text-center text-[30px] tracking-normal">{plan.name}</h2>
      <p className="mx-auto mb-6 mt-0 max-w-[250px] text-center text-sm leading-[1.18] text-[#bdbdbd]">{plan.desc}</p>
      <div className="mt-0.5 flex items-baseline gap-4">
        <strong className={`text-[clamp(46px,4vw,50px)] leading-none ${isPremium ? "text-[#ffd54f]" : "text-white"}`}>{getPlanMonthlyLabel(plan).replace("฿", "")}฿</strong>
      ...
      <ul className="mb-7 mt-0 grid list-none gap-[17px] p-0">
        {(plan.features || []).map((feature) => (
          <li className="flex items-center gap-3 text-sm text-[#e4e4e4]" key={feature}>
            <span className={`grid h-5 w-5 flex-[0_0_20px] place-items-center rounded-full text-xs font-black ${isPremium ? "bg-[#ffd54f] text-[#111]" : "bg-[#e6002e] text-white"}`}>✓</span>
            {feature}
          </li>
        ))}
      </ul>
      ...
      <button
        ...
        type="button"
      >
        {isStandard ? "Choose Standard" : "Get Started"}
      </button>
    </article>
  );
}
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#0d0d0d] px-[clamp(28px,5vw,70px)] pb-6 pt-[38px] font-[Inter,Arial,sans-serif] text-white max-[1020px]:overflow-auto max-[640px]:p-4">
      ...
      <header className={flowNav}>
        ...
        <p className="m-0 text-[13px] font-black text-[#bdbdbd]">Step 2: Choose your membership plan</p>
      </header>

      <section className={`${pageContent} py-4 pb-[18px] text-center max-[640px]:py-7 max-[640px]:text-left`}>
        <span className="mb-[3px] block text-[13px] font-black uppercase text-[#e6002e]">Membership Setup</span>
        <h1 className="mb-1 mt-0 text-[clamp(42px,4.4vw,48px)] leading-[1.05] tracking-normal">Choose Your Plan</h1>
        <p className="m-0 text-[17px] text-[#bdbdbd]">Select the plan that matches your fitness goal. You can upgrade anytime.</p>
      </section>

      {status === "loading" && <p className={`${pageContent} py-20 text-center text-[17px] text-[#bdbdbd]`}>Loading membership plans...</p>}
      {status === "error" && <p className={`${pageContent} py-20 text-center text-[17px] text-[#ff8ea2]`}>Membership plans are unavailable right now.</p>}
      {status === "ready" && plans.length === 0 && <p className={`${pageContent} py-20 text-center text-[17px] text-[#bdbdbd]`}>No active membership plans are available.</p>}

      {status === "ready" && plans.length > 0 && (
        <section className={`${pageContent} grid grid-cols-3 items-center gap-[clamp(32px,5vw,76px)] pt-0 max-[1020px]:grid-cols-[minmax(0,520px)] max-[1020px]:justify-center`}>
          {plans.map((plan) => (
